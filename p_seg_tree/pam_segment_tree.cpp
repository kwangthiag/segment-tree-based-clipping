#include "segment_tree/pam_segment_tree.h" // The new header file using PAM
#include <omp.h>
...
void SegTree::insertEdgeParallel(const Edge& x, int nodeIdx) {
    ...
    if (x.contains(node_interval.start, node_interval.end)) {
    ...
    if (leftIdx <= treeSize) {
        ...
        if (x.intersects(left_interval.start, left_interval.end)) {
    ...
    if (rightIdx <= treeSize) {
        ...
        if (x.intersects(right_interval.start, right_interval.end)) {
