...
vector<polygon> bPolysList, cPolysList; 
// vertex **PPVertexPointers, **QQVertexPointers;                // two input polygons
vector<polygon> rPolyList;                     // output polygon
...
void computeIntersections() {
  ...
  //
  for (polygon& P : bPolysList) 
    for (edge edgeP : P.edges(SOURCE))
      for (polygon& Q : cPolysList) 
...
//
void labelIntersections2(int bListId, int cListId) {
  #ifdef DG3 
  ...
  count[0] = count[1] = 0;
  polygon& P=bPolysList[bListId];
  polygon& Q=cPolysList[cListId];

  // loop over intersection vertices of P
  // for (polygon& P : bPolysList) // use this to label more than one polygon pair
    for (vertex* I : P.vertices(INTERSECTION)) {
      ...
      // start of an intersection chain ?
        ...
        // cout << "lbl2 "<<I->p.x<<","<<I->p.y<<" chainType="<<chainType<<" "<<X->p.x<<","<<X->p.y<<" chainType="<<chainType<<endl;
  		}
    }
  ...
  for (int i=0; i<2; ++i) {
    vector<polygon>* P_or_Q = &bPolysList;      // if i=0, then do it for P w.r.t. Q
    vector<polygon>* Q_or_P = &cPolysList;
    int pqId = bListId;
    int qpId=cListId;
    ...
    // for (polygon& P : *P_or_Q)
      if (P.noCrossingVertex(UNION)) {
        ...
        if (P.allOnOn()) {
          ...
          polygon& Q=Q_or_P->at(qpId);
          // for (polygon& Q : *Q_or_P)
            if ( Q.pointInPoly(p) )
              isInside = !isInside;              
          if (isInside ^ UNION) {
            rPolyList.push_back(P);             // -> add P to the result
            count[0]++;
          }
        }
      }
  ...
  for (polygon* P : identical[0]) {
    // is P a hole?
    bool P_isHole = false;
    polygon& P_=bPolysList[bListId];
    // for (polygon& P_ : bPolysList)
    ...
    for (polygon* Q : identical[1])
      for (vertex* V : Q->vertices(ALL))
        if (V == P->root->neighbour) {  // found Q that matches P
          ...
          polygon& Q_=cPolysList[cListId];
          // for (polygon& Q_ : cPolysList)
...
//
void printInfo(vector<polygon>& bPolysList) {
  int sum = 0;
  cout << "has "<< bPolysList.size() << " component";
  if (bPolysList.size() > 1) 
    cout << "s";
  cout << " with ";
  for (int i=0; i<bPolysList.size(); i++) {
    int count = 0;
    for (vertex* V : bPolysList[i].vertices(ALL))
      count++;
    cout << count;
    if (i<bPolysList.size()-1) 
      cout << " + ";
    sum += count;
  }
  if (bPolysList.size() > 1)
...
//
void loadPolygon(vector<polygon>& bPolysList, string s) {
  ifstream from(s);
  do {
    polygon P;
    from >> P;
    if (!from.fail())
      bPolysList.push_back(P);
  } while (!from.eof());
  from.close();
  #ifdef DG3 
  printInfo(bPolysList);
  #endif
}
//
////////////////////////////////////////////////////////////////////////
...
//
void savePolygon(vector<polygon>& bPolysList, string s) {
  ofstream to(s);
  for (polygon& P : bPolysList)
    to << P;
  to.close();
...
void savePolygon2(vector<polygon>& bPolysList, string s) {
  printInfoCounts(bPolysList);
}